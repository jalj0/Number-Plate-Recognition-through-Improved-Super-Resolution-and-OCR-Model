...
def train_test_split(path):
    ...
    with open(path, 'r+', encoding='utf8') as fp:
        ...
        for line in p:
            ...
            with open(ocrTxt, encoding='utf8') as fp:
                ...
                # print(tp)
                fp.seek(0)
                plate = fp.readlines()[0].split(':')[1].replace('\n', '').replace(' ', '')
                # print(plate)
                fp.seek(0)
                layout = fp.readlines()[1].split(':')[1].replace('\n', '').replace(' ', '')
                # print(layout)
            
            # print("line", imgHR, imgLR, plate, layout,tp)
    ...
    # print("split done")
    # print(train[0],test[0],validate[0])
    return (train, test, validate)

class customDataset(Dataset):
    def __init__(self, x_tensor, augmentation = True):
        # x_tensor is train[],val[]
        self.x = x_tensor
        self.to_tensor = transforms.ToTensor()
        self.augmentation = True
        
        self.background_color = (127, 127, 127) #padding ka bg color->grey
...
def rodosol_dataset(path, dest):
    ...
    split_text = Path('./3xgFxD-CG-SRPlates-Datasets/Rodosol-ALPR-SR/split.txt')
...
if __name__ == '__main__':
    ...
    print("here")
    rodosol_dataset('./3xgFxD-CG-SRPlates-Datasets/Rodosol-ALPR-SR/split.txt', '/content/drive/MyDrive/SR_of_LP_using_attention_and_subpixel/Proposed/3xgFxD-CG-SRPlates-Datasets/Rodosol-ALPR-SR')
    print("here 2")
