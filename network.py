...
class ResidualConcatenationBlock(nn.Module):
    ...
    def forward(self, x):
        for i in range(1, self.num_layers+1):
            ...
            x = self.pw[i-1](x)
        conv = nn.Conv2d(x.shape[1], self.out_channels, kernel_size=1, stride=1, padding=0, bias=False).to(x.device)
        x=conv(x)
...
class Network(nn.Module):
    ...
    def forward(self, x):
        ...
        output = F.sigmoid(output + F.interpolate( x, (output.size(-2), output.size(-1)), mode='bicubic'))
...
if __name__ == "__main__":
    ...
    net = Network(3, 3)
    # print(net)
    output = net(feature_maps)
    print(output.shape)
    # # freeze layers
    # for param in net.parameters():
    #     param.requires_grad = False

    # # Replace some of the final layers
    # net.upscale = nn.Sequential(nn.Conv2d(128, 128*2**2, kernel_size=3, stride=1, padding='same', bias=False),
    #                                 nn.PixelShuffle(2),
    #                                 nn.Conv2d(128, 128*2**2, kernel_size=3, stride=1, padding='same', bias=False),
    #                                 nn.PixelShuffle(2))
    # net.Output = nn.Sequential(nn.Conv2d(128, 3, kernel_size=3, stride=1, padding='same', bias=False))

    # # make new layers trainable
    # for param in net.upscale.parameters():
    #     param.requires_grad = True
    # for param in net.Output.parameters():
    #     param.requires_grad = True

    # Display model summary using torchsummary
    # summary(net, input_size=(3, 40, 20), device='cpu')

    '''Network DEBUG'''
    # net = Network(3, 3)
    # print(net)
    # print('Network shapes', net(feature_maps).shape)
    # total_params = sum(p.numel() for p in net.parameters() if p.requires_grad)  # Calculate the total number of trainable parameters
